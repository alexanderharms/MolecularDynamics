...
from moleculardynamics.base import Environment, SimulationEnsemble
from moleculardynamics.vis import plot_energy, plot_g
from moleculardynamics.measurements import calc_heat_capacity, calc_histogram, calc_g
...
# Temperature [120*K]. Temperature will be regulated until system is in equillibrium [120*K]
temp = 1.462 
...
ensemble = SimulationEnsemble(num_particles, envir)
...
for step in range(num_steps):
    ...
    ensemble.take_step(dt)
    kin_energy[step] = ensemble.kin_energy
    pot_energy[step] = ensemble.pot_energy
    hist_array[step, :] = calc_histogram(ensemble.pos, envir.dimens, num_bins)

plot_energy(num_steps, dt, kin_energy, pot_energy)
cv, _ = calc_heat_capacity(kin_energy, num_particles, block_size, num_blocks)

# Calculate g
# Delete timesteps before equilibrium
hist_array = np.delete(hist_array, np.s_[0:t_equilibrium], axis = 0) 
g_mean, g_error, r = calc_g(hist_array, ensemble.num_particles,
